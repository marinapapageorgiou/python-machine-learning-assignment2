...
if __name__ == '__main__':
    ...
    # for example
    y_pred_task1 = model_task1.predict(test_norm)


    # Evaluate the missclassification error on the test set
    # for example
    assert testY.shape == y_pred_task1.shape
    acc = CategoricalAccuracy()
    acc.update_state(testY, y_pred_task1)
    acc1 = acc.result()
    print("Accuracy model task 1:", float(acc1))
